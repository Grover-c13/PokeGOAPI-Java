...
import com.debug.pokegoapi.exceptions.AsyncPokemonGoException;
import com.debug.pokegoapi.exceptions.LoginFailedException;
import com.debug.pokegoapi.exceptions.RemoteServerException;
import com.debug.pokegoapi.util.AsyncHelper;
import com.debug.pokegoapi.util.Log;
import com.debug.pokegoapi.util.Signature;
...
public class RequestHandler implements Runnable {
	 ...
	 */
	public RequestHandler(InfoHolder infoHolder, OkHttpClient client) {
	...
	public Observable<ByteString> sendAsyncServerRequests(final AsyncServerRequest serverRequest) {
		...
		return Observable.from(new Future<ByteString>() {
			...
			private ResultOrException getResult(long timeout, TimeUnit timeUnit) throws InterruptedException {
				long wait = System.currentTimeMillis() + timeUnit.toMillis(timeout);
				while (!isDone()) {
					Thread.sleep(10);
	...
	private AuthTicket internalSendServerRequests(AuthTicket authTicket, ServerRequest... serverRequests)
		...
		resetBuilder(infoHolder, builder, authTicket);
		boolean map = false;
		for (ServerRequest serverRequest : serverRequests) {
			if (serverRequest.getType() == RequestTypeOuterClass.RequestType.GET_MAP_OBJECTS) {
				map = true;
			}
		...
		Signature.setSignature(infoHolder, builder);
	...
	private void resetBuilder(InfoHolder infoHolder, RequestEnvelope.Builder builder, AuthTicket authTicket)
		...
		if (authTicket != null
				...
				&& authTicket.getExpireTimestampMs() > System.currentTimeMillis()) {
			builder.setAuthTicket(authTicket);
		} else {
			Log.d(TAG, "Authenticated with static token");
			builder.setAuthInfo(infoHolder.getAuthInfo());
		}
		builder.setMsSinceLastLocationfix(989);
		builder.setLatitude(infoHolder.getLatitude());
		builder.setLongitude(infoHolder.getLongitude());
		builder.setAltitude(infoHolder.getAltitude());
	}
	...
	public void run() {
		...
		while (true) {
			...
			try {
				if (authTicket == null) {
					authTicket = RequestHandler.this.infoHolder.getAuthTicket();
				}
